...
export default function DashboardPage() {
  ...
  React.useEffect(() => {
    ...
    if (connected) {
      toast({ title: 'Connected', description: `Your ${connected} inbox is linked. Next: run your first sync.` });
      mutateMailboxes();
      mutate();
      ...
      toast({ variant: 'destructive', title: 'Connection didn’t complete', description: `We couldn’t finish linking your account.${errorCode ? ` Code: ${errorCode}` : ''}` });
    }
  }, [toast, mutateMailboxes, mutate]);
  ...
  const hasConnectedMailbox = mailboxes.length > 0;
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-8">
              ...
              <CardHeader>
                <CardTitle className="font-headline text-base flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    {m.isActive && (
                      <span className="ml-2 rounded bg-green-100 px-2 py-0.5 text-xs text-green-700">Active</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <Tooltip>
                ...
                <CardDescription className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {!m.isActive && (
                      <Button size="sm" variant="outline" onClick={() => setActive(m.id)}>Set Active</Button>
                    )}
      ...
      {isLoading ? (
        ...
        <>
          {!hasConnectedMailbox && <ConnectMailbox />}
          {hasConnectedMailbox && (
             <div className="mt-8 mb-4">
               <h2 className="font-headline text-lg mb-2">Connect another mailbox</h2>
               <ConnectMailbox />
             </div>
          )}
          {hasConnectedMailbox && domains.length === 0 && (
            <Card className="mt-6">
              <CardHeader>
                <CardTitle className="font-headline">Connected ✅ Next: run your first sync</CardTitle>
                <CardDescription>We’re ready to scan your inbox and build your account list. This usually takes a few minutes.</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          {domains.length > 0 && <DomainTable domains={domains} />}
        </>
      )}
    </div>
  );
