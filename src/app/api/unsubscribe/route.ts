...
export async function POST(req: NextRequest) {
  try {
    ...
    if (inventoryId) {
      const { data: invs } = await (await inventoryTable()).select('*').eq('id', inventoryId).limit(1);
      const inv = (invs && invs[0]) as Inventory | undefined;
      if (!inv) return NextResponse.json({ error: 'Inventory not found' }, { status: 404 });
      const { data: mbs } = await (await mailboxesTable()).select('*').eq('id', inv.mailboxId).limit(1);
